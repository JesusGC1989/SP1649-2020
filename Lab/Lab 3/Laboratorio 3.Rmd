...
Se carga la base de datos sobre la lluvia en California. 
```{r, echo=TRUE, hide=TRUE, cache= TRUE}
if (!require("rspatial")) devtools::install_github('rspatial/rspatial')
library(rspatial)
d <- sp_data('precipitation')
head(d)
```
El gráfico muestra la cantidad de precipitación anual en los diversos puntos, según las estaciones.
```{r, echo=TRUE, hide=TRUE, cache= TRUE}
d$prec <- rowSums(d[, c(6:17)])
plot(sort(d$prec), ylab='Annual precipitation (mm)', las=1, xlab='Stations')
```
Se hace un mapa para ver su distribución en el mapa de California. 
```{r}
library(sp)
...
```
Se hacen compatibles los datos, y se hace la transformación en sp
...
```
Se crea la función para el cálculo del RMSEA. Se toma la media como el modelo nulo, y se interpolan los puntos que no existen con este modelo. 
```{r}
...
plot(v)
```
La figura anterior se limita a Califoria. En este caso se divide el espacil en polígonos
```{r}
ca <- aggregate(cata)
vca <- intersect(v, ca)
spplot(vca, 'prec', col.regions=rev(get_col_regions()))
```
y los podemos crear como un raster
```{r}
...
Acá usamos el método de la distancia ponderada inversa.
```{r}
library(gstat)
...
# Cargar base de contaminación del aire
